...
if len(sys.argv) >= 3:
    SINGLE_DYNAMIC_MODE = int(sys.argv[2])
else:
    SINGLE_DYNAMIC_MODE = 1

if len(sys.argv) >= 4:
    DENSITY_MULTIPLIER = float(sys.argv[3])
else:
...
class World:
    ...
    def toYaml(self, filename):
        ...
        with open(filename, "w") as f:
            # before all dynamic objects yaml.dump(d, f), now only one dynamic is made
            if SINGLE_DYNAMIC_MODE:
                yaml.dump({'N': 1, "Object1": d["Object1"]}, f)
            else:
                yaml.dump(d, f)
